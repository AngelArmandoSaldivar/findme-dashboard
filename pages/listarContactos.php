...
include 'conection.php';
            ...
            $consulta=" SELECT * FROM contact LIMIT 0";
			$termino= "";
			if(isset($_POST['contactos']))
			{
				$termino=$conection->real_escape_string($_POST['contactos']);
				$consulta="SELECT * FROM contact WHERE
			...
			if($consultaBD->num_rows>=1){
				...
				while($fila=$consultaBD->fetch_array(MYSQLI_ASSOC)){
							echo "<tr>
								...
								<td>".$fila['phone']."</td>
                                <td>".$fila['age']."</td>
								<td><a class='btn btn-success btn-xs' href='contactoView.php?id=".$fila['idcontact']."'><i class='fas fa-eye'></i></a> 
								<a class='btn btn-warning btn-xs'  href='contactEdit.php?id=".$fila['idcontact']."'><i class='fas fa-pencil-alt'></i></a>
								<a class='btn btn-danger btn-xs'  id='btnElim' href='sql_EliminarContact.php?id=".$fila['idcontact']."'><i class='fas fa-trash-alt'></i></a>
							</tr>";
				}
				echo "</tbody>
				</table><script src=confirm.js'></script>";
			}else{
